...
const terminalLines = [
	{ type: "command", text: "$ nbench run --engineer @kwao" },
	{ type: "output", text: "" },
		...
		<div>
			...
			<GridSection columns="1fr" border>
				<GridCell className="px-12 py-10">
					...
					<h2 className="text-4xl md:text-[48px] md:leading-[57.6px] font-normal tracking-[-0.48px] text-[#E8E4DE] max-w-3xl mb-3">
						We built{" "}
						<span className="font-serif italic text-[#C9A96E]">NBench</span>{" "}
						to prove it
					</h2>
					<p className="text-[#A39E96] text-base max-w-xl">
						An open-source CLI that measures AI-nativeness across the three dimensions that matter. Every engineer in our network runs it.
					</p>
				</GridCell>
			</GridSection>
			...
			<GridSection columns="1fr 1fr" border>
				...
				<GridCell borderRight className="p-0">
					<div className="h-full bg-[#0A0A0A] font-mono text-[13px] leading-relaxed overflow-hidden">
						{/* Terminal header */}
						<div className="flex items-center gap-2 px-5 py-3 border-b border-white/6">
							<div className="w-3 h-3 rounded-full bg-[#FF5F57]" />
							<div className="w-3 h-3 rounded-full bg-[#FEBC2E]" />
							<div className="w-3 h-3 rounded-full bg-[#28C840]" />
							<span className="ml-3 text-[11px] text-[#A39E96]/50">
								nbench
							</span>
						</div>
						...
						<div className="px-5 py-4 space-y-0.5">
							{terminalLines.map((line, i) => {
								if (line.type === "command") {
									return (
										<div key={i} className="text-[#E8E4DE]">
											{line.text}
										</div>
									);
								}
								...
								if (line.type === "result") {
									return (
										<div key={i} className="text-[#C9A96E] font-semibold">
											{line.text}
										</div>
									);
								}
								return (
									<div key={i} className="text-[#A39E96]/60 min-h-[1.5em]">
										{line.text}
									</div>
								);
							})}
