		...
		<div className="relative rounded-2xl overflow-hidden h-full min-h-[400px]">
			...
			<div className="relative h-full flex flex-col justify-end p-8">
				...
				<div className="flex items-center gap-2">
					...
					<p className={colors.textMuted}>{PROGRAM_QUOTE.role}</p>
				</div>
			</div>
		</div>
	);
...
function StatCard({
	...
	return (
		<GlassCard className="p-6 flex flex-col justify-between min-h-[220px]">
			<h4 className={`text-sm font-medium ${colors.textMuted}`}>
				{label}
			</h4>
			<div className="flex items-end justify-between mt-auto">
				<div className="flex items-baseline gap-1">
					...
					{suffix && (
						<span className="text-2xl text-white/70 italic">
							{suffix}
						</span>
					)}
				</div>
				<p className={`text-sm text-right ${colors.textMuted}`}>
					{sublabel}
				</p>
			</div>
		</GlassCard>
...
function StatsRow() {
	return (
		<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-12">
			...
			<div className="pricing-gradient-green rounded-2xl overflow-hidden p-6 flex flex-col justify-between min-h-[220px]">
				<div>
					<div className="flex items-center gap-1 mb-1">
						{PROGRAM_STATS.avatars.map((src) => (
							<img
								...
								src={src}
								alt="Avatar"
								className="w-8 h-8 rounded-full border-2 border-black -ml-2 first:ml-0 object-cover"
							/>
						))}
						<span className="text-white/70 text-sm ml-1 font-medium">
